...
function performMoves(crates: string[][], moves: MoveInstruction[]): string[][] {
    for (const move of moves) {
        const sourceCrates = crates[move.source - 1].splice(
            crates[move.source - 1].length - move.amount,
            move.amount
        );
        crates[move.destination - 1] = crates[move.destination - 1].concat(sourceCrates);
    }
